...
DONE: - machine:
   halui2.machine.on                    bit  //pin for setting machine On
   halui2.machine.off                   bit  //pin for setting machine Off
   halui2.machine.is-on                 bit  //pin for machine is On/Off

DONE: - estop:   
   halui2.estop.activate                bit  //pin for setting Estop (emc internal) On
   halui2.estop.reset                   bit  //pin for resetting Estop (emc internal) Off
   halui2.estop.is-activated            bit  //pin for displaying Estop state (emc internal) On/Off
   
DONE: - mode:
   halui2.mode.manual                   bit  //pin for requesting manual mode
   halui2.mode.is_manual                bit  //pin for manual mode is on
   halui2.mode.auto                     bit  //pin for requesting auto mode
   halui2.mode.is_auto                  bit  //pin for auto mode is on
   halui2.mode.mdi                      bit  //pin for requesting mdi mode
   halui2.mode.is_mdi                   bit  //pin for mdi mode is on
   halui2.mode.teleop                   bit  //pin for requesting teleop mode
   halui2.mode.is_teleop                bit  //pin for teleop mode is on
   halui2.mode.joint                    bit  //pin for requesting joint mode
   halui2.mode.is_joint                 bit  //pin for joint mode is on

DONE: - mist, flood, lube:
   halui2.mist.on                       bit  //pin for starting mist
   halui2.mist.off                      bit  //pin for stoping mist
   halui2.mist.is-on                    bit  //pin for mist is on
   halui2.flood.on                      bit  //pin for starting flood
   halui2.flood.off                     bit  //pin for stoping flood
   halui2.flood.is-on                   bit  //pin for flood is on
   halui2.lube.on                       bit  //pin for starting lube
   halui2.lube.off                      bit  //pin for stoping lube
   halui2.lube.is-on                    bit  //pin for lube is on

DONE: - spindle:
   halui2.spindle.start                 bit
   halui2.spindle.stop                  bit
   halui2.spindle.forward               bit
   halui2.spindle.reverse               bit
   halui2.spindle.increase              bit
   halui2.spindle.decrease              bit

   halui2.spindle.brake-on              bit  //pin for activating spindle-brake
   halui2.spindle.brake-off             bit  //pin for deactivating spindle/brake
   halui2.spindle.brake-is-on           bit  //status pin that tells us if brake is on

DONE: - joint:
   halui2.joint.0.home                  bit  // pin for homing the specific joint
   halui2.joint.0.unhome                bit  // pin for unhoming the specific joint
   halui2.joint.0.is-homed              bit  // status pin telling that the joint is homed
   ..
   halui2.joint.8.home                  bit 
   halui2.joint.8.is-homed              bit 

   halui2.joint.selected.home           bit  // pin for homing the selected joint
   halui2.joint.selected.unhome         bit  // pin for unhoming the selected joint
   halui2.joint.selected.is-homed       bit  // status pin telling that the selected joint is homed

   halui2.joint.x.on-soft-min-limit     bit
   halui2.joint.x.on-soft-max-limit     bit
   halui2.joint.x.on-hard-min-limit     bit
   halui2.joint.x.on-hard-max-limit     bit
     (x = 0..8, selected)
   
   halui2.joint.x.has-fault             bit   
     (x = 0..8, selected)

   halui2.joint.select                  u8   // select joint (0..7)           - internal halui
   halui2.joint.selected                u8   // selected joint (0..7)         - internal halui
   halui2.joint.x.select                bit  // pins for selecting a joint    - internal halui
   halui2.joint.x.is-selected           bit  // status pin                    - internal halui

WIP:
   halui2.axis.0.pos-commanded          float //cartesian position, commanded
   halui2.axis.0.pos-feedback           float //cartesian position, actual
   halui2.axis.0.pos-relative           float //cartesian position, relative
   ...

DONE - jogging:
   halui2.jog-speed                     float //set jog speed
   halui2.jog-deadband                  float //pin for setting the jog analog deadband (where not to move)

   halui2.jog.0.minus                   bit
   halui2.jog.0.plus                    bit
   halui2.jog.0.analog                  float //pin for jogging the axis 0
   halui2.jog.0.increment               float
   halui2.jog.0.increment-plus          bit
   halui2.jog.0.increment-minus         bit
   ..
   halui2.jog.7.minus                   bit
   halui2.jog.7.plus                    bit
   halui2.jog.7.analog                  float //pin for jogging the axis 7
   halui2.jog.7.increment               float
   halui2.jog.7.increment-plus          bit
   halui2.jog.7.increment-minus         bit
   halui2.jog.selected.minus            bit
   halui2.jog.selected.plus             bit
   halui2.jog.selected.increment        float
   halui2.jog.selected.increment-plus   bit
   halui2.jog.selected.increment-minus  bit

DONE - tool:
   halui2.tool.number                   u32  //current selected tool
   halui2.tool.length-offset            float //current applied tool-length-offset

DONE: - program:
   halui2.program.is-idle               bit
   halui2.program.is-running            bit
   halui2.program.is-paused             bit
   halui2.program.run                   bit
   halui2.program.pause                 bit
   halui2.program.resume                bit
   halui2.program.step                  bit
   halui2.program.stop                  bit

DONE: - general:
   halui2.home-all                      bit // pin to send a sequenced home all joints message
   halui2.abort                         bit // pin to send an abort message (clears out most errors, stops running programs, etc)

DONE: - max-velocity-override
   halui2.max-velocity-override.value           float //current MV value
   halui2.max-velocity-override.scale           float // pin for setting the scale on changing the MV
   halui2.max-velocity-override.counts          s32   //counts from an encoder for example to change MV
   halui2.max-velocity-override.count-enable    bit   // TRUE to modify MV based on counts
   halui2.max-velocity-override.increase        bit   // pin for increasing the MV (+=scale)
   halui2.max-velocity-override.direct-value    bit   // TRUE to make override based as a direct (scaled) value rather then counts of increments
   halui2.max-velocity-override.decrease        bit   // pin for decreasing the MV (-=scale)

DONE: - feed-override
   halui2.feed-override.value           float //current FO value
   halui2.feed-override.scale           float // pin for setting the scale on changing the FO
   halui2.feed-override.counts          s32   //counts from an encoder for example to change FO
   halui2.feed-override.count-enable    bit   // TRUE to modify FO based on counts
   halui2.feed-override.direct-value    bit   // TRUE to make override based as a direct (scaled) value rather then counts of increments
   halui2.feed-override.increase        bit   // pin for increasing the FO (+=scale)
   halui2.feed-override.decrease        bit   // pin for decreasing the FO (-=scale)

DONE: - rapid-override
   halui2.rapid-override.value           float //current RO value
   halui2.rapid-override.scale           float // pin for setting the scale on changing the RO
   halui2.rapid-override.counts          s32   //counts from an encoder for example to change RO
   halui2.rapid-override.count-enable    bit   // TRUE to modify RO based on counts
   halui2.rapid-override.direct-value    bit   // TRUE to make override based as a direct (scaled) value rather then counts of increments
   halui2.rapid-override.increase        bit   // pin for increasing the RO (+=scale)
   halui2.rapid-override.decrease        bit   // pin for decreasing the RO (-=scale)

DONE: - spindle-override
   halui2.spindle-override.value           float //current FO value
   halui2.spindle-override.scale           float // pin for setting the scale on changing the SO
   halui2.spindle-override.counts          s32   //counts from an encoder for example to change SO
   halui2.spindle-override.count-enable    bit   // TRUE to modify SO based on counts
   halui2.spindle-override.direct-value    bit   // TRUE to make override based as a direct (scaled) value rather then counts of increments
   halui2.spindle-override.increase        bit   // pin for increasing the SO (+=scale)
   halui2.spindle-override.decrease        bit   // pin for decreasing the SO (-=scale)
...
int halui_hal_init(void)
{
    ...
    retval = halui_export_pin_OUT_bit(&(halui_data->machine_is_on), "halui2.machine.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->estop_is_activated), "halui2.estop.is-activated"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mode_is_manual), "halui2.mode.is-manual"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mode_is_auto), "halui2.mode.is-auto"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mode_is_mdi), "halui2.mode.is-mdi"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mode_is_teleop), "halui2.mode.is-teleop"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mode_is_joint), "halui2.mode.is-joint"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->mist_is_on), "halui2.mist.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->flood_is_on), "halui2.flood.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->lube_is_on), "halui2.lube.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->program_is_idle), "halui2.program.is-idle"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->program_is_running), "halui2.program.is-running"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->program_is_paused), "halui2.program.is-paused"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->program_os_is_on), "halui2.program.optional-stop.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->program_bd_is_on), "halui2.program.block-delete.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->spindle_is_on), "halui2.spindle.is-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->spindle_runs_forward), "halui2.spindle.runs-forward"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->spindle_runs_backward), "halui2.spindle.runs-backward"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_OUT_bit(&(halui_data->spindle_brake_is_on), "halui2.spindle.brake-is-on"); 
    if (retval < 0) return retval;

    for (joint=0; joint < num_axes ; joint++) {
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_is_homed[joint]), comp_id, "halui2.joint.%d.is-homed", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_is_selected[joint]), comp_id, "halui2.joint.%d.is-selected", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_soft_min_limit[joint]), comp_id, "halui2.joint.%d.on-soft-min-limit", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_soft_max_limit[joint]), comp_id, "halui2.joint.%d.on-soft-max-limit", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_hard_min_limit[joint]), comp_id, "halui2.joint.%d.on-hard-min-limit", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_hard_max_limit[joint]), comp_id, "halui2.joint.%d.on-hard-max-limit", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_has_fault[joint]), comp_id, "halui2.joint.%d.has-fault", joint); 
	if (retval < 0) return retval;
    }

    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_soft_min_limit[num_axes]), comp_id, "halui2.joint.selected.on-soft-min-limit"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_soft_max_limit[num_axes]), comp_id, "halui2.joint.selected.on-soft-limit"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_hard_min_limit[num_axes]), comp_id, "halui2.joint.selected.on-hard-min-limit"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_on_hard_max_limit[num_axes]), comp_id, "halui2.joint.selected.on-hard-max-limit"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_has_fault[num_axes]), comp_id, "halui2.joint.selected.has-fault"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_OUT, &(halui_data->joint_is_homed[num_axes]), comp_id, "halui2.joint.selected.is_homed"); 
    if (retval < 0) return retval;

    for (axis=0; axis < EMCMOT_MAX_AXIS ; axis++) {
	retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->axis_pos_commanded[axis]), comp_id, "halui2.axis.%d.pos-commanded", axis);
    if (retval < 0) return retval;
	retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->axis_pos_feedback[axis]), comp_id, "halui2.axis.%d.pos-feedback", axis);
    if (retval < 0) return retval;
	retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->axis_pos_relative[axis]), comp_id, "halui2.axis.%d.pos-relative", axis);
    if (retval < 0) return retval;
    }

    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->mv_value), comp_id, "halui2.max-velocity.value"); 
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->fo_value), comp_id, "halui2.feed-override.value"); 
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->ro_value), comp_id, "halui2.rapid-override.value"); 
    if (retval < 0) return retval;
    retval = hal_pin_u32_newf(HAL_OUT, &(halui_data->joint_selected), comp_id, "halui2.joint.selected"); 
    if (retval < 0) return retval;
    retval = hal_pin_u32_newf(HAL_OUT, &(halui_data->tool_number), comp_id, "halui2.tool.number"); 
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_x), comp_id, "halui2.tool.length_offset.x");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_y), comp_id, "halui2.tool.length_offset.y");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_z), comp_id, "halui2.tool.length_offset.z");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_a), comp_id, "halui2.tool.length_offset.a");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_b), comp_id, "halui2.tool.length_offset.b");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_c), comp_id, "halui2.tool.length_offset.c");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_u), comp_id, "halui2.tool.length_offset.u");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_v), comp_id, "halui2.tool.length_offset.v");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->tool_length_offset_w), comp_id, "halui2.tool.length_offset.w");
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_OUT, &(halui_data->so_value), comp_id, "halui2.spindle-override.value"); 
    if (retval < 0) return retval;

    /* STEP 3b: export the in-pin(s) */

    retval = halui_export_pin_IN_bit(&(halui_data->machine_on), "halui2.machine.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->machine_off), "halui2.machine.off"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->estop_activate), "halui2.estop.activate"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->estop_reset), "halui2.estop.reset"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mode_manual), "halui2.mode.manual"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mode_auto), "halui2.mode.auto"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mode_mdi), "halui2.mode.mdi"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mode_teleop), "halui2.mode.teleop"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mode_joint), "halui2.mode.joint"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mist_on), "halui2.mist.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mist_off), "halui2.mist.off"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->flood_on), "halui2.flood.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->flood_off), "halui2.flood.off"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->lube_on), "halui2.lube.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->lube_off), "halui2.lube.off"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_run), "halui2.program.run"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_pause), "halui2.program.pause"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_resume), "halui2.program.resume"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_step), "halui2.program.step"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_stop), "halui2.program.stop"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_os_on), "halui2.program.optional-stop.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_os_off), "halui2.program.optional-stop.off"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_bd_on), "halui2.program.block-delete.on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->program_bd_off), "halui2.program.block-delete.off"); 
    if (retval < 0) return retval;

    retval = halui_export_pin_IN_bit(&(halui_data->spindle_start), "halui2.spindle.start");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_stop), "halui2.spindle.stop");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_forward), "halui2.spindle.forward");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_reverse), "halui2.spindle.reverse");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_increase), "halui2.spindle.increase");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_decrease), "halui2.spindle.decrease");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_brake_on), "halui2.spindle.brake-on"); 
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->spindle_brake_off), "halui2.spindle.brake-off"); 
    if (retval < 0) return retval;

    retval = halui_export_pin_IN_s32(&(halui_data->mv_counts), "halui2.max-velocity.counts");
    if (retval < 0) return retval;
    *halui_data->mv_counts = 0;
    retval = halui_export_pin_IN_bit(&(halui_data->mv_count_enable), "halui2.max-velocity.count-enable");
    if (retval < 0) return retval;
    *halui_data->mv_count_enable = 1;
    retval = halui_export_pin_IN_bit(&(halui_data->mv_direct_value), "halui2.max-velocity.direct-value");
    if (retval < 0) return retval;
    *halui_data->mv_direct_value = 0;
    retval = halui_export_pin_IN_float(&(halui_data->mv_scale), "halui2.max-velocity.scale");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mv_increase), "halui2.max-velocity.increase");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->mv_decrease), "halui2.max-velocity.decrease");
    if (retval < 0) return retval;

    retval = halui_export_pin_IN_s32(&(halui_data->fo_counts), "halui2.feed-override.counts");
    if (retval < 0) return retval;
    *halui_data->fo_counts = 0;
    retval = halui_export_pin_IN_bit(&(halui_data->fo_count_enable), "halui2.feed-override.count-enable");
    if (retval < 0) return retval;
    *halui_data->fo_count_enable = 1;
    retval = halui_export_pin_IN_bit(&(halui_data->fo_direct_value), "halui2.feed-override.direct-value");
    if (retval < 0) return retval;
    *halui_data->fo_direct_value = 0;
    retval = halui_export_pin_IN_float(&(halui_data->fo_scale), "halui2.feed-override.scale");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->fo_increase), "halui2.feed-override.increase");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->fo_decrease), "halui2.feed-override.decrease");
    if (retval < 0) return retval;

    retval = halui_export_pin_IN_s32(&(halui_data->ro_counts), "halui2.rapid-override.counts");
    if (retval < 0) return retval;
    *halui_data->ro_counts = 0;
    retval = halui_export_pin_IN_bit(&(halui_data->ro_count_enable), "halui2.rapid-override.count-enable");
    if (retval < 0) return retval;
    *halui_data->ro_count_enable = 1;
    retval = halui_export_pin_IN_bit(&(halui_data->ro_direct_value), "halui2.rapid-override.direct-value");
    if (retval < 0) return retval;
    *halui_data->ro_direct_value = 0;
    retval = halui_export_pin_IN_float(&(halui_data->ro_scale), "halui2.rapid-override.scale");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->ro_increase), "halui2.rapid-override.increase");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->ro_decrease), "halui2.rapid-override.decrease");
    if (retval < 0) return retval;

    retval = halui_export_pin_IN_s32(&(halui_data->so_counts), "halui2.spindle-override.counts");
    if (retval < 0) return retval;
    *halui_data->so_counts = 0;
    retval = halui_export_pin_IN_bit(&(halui_data->so_count_enable), "halui2.spindle-override.count-enable");
    if (retval < 0) return retval;
    *halui_data->so_count_enable = 1;
    retval = halui_export_pin_IN_bit(&(halui_data->so_direct_value), "halui2.spindle-override.direct-value");
    if (retval < 0) return retval;
    *halui_data->so_direct_value = 0;
    retval = halui_export_pin_IN_float(&(halui_data->so_scale), "halui2.spindle-override.scale");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->so_increase), "halui2.spindle-override.increase");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_bit(&(halui_data->so_decrease), "halui2.spindle-override.decrease");
    if (retval < 0) return retval;

    if (have_home_all) {
        retval = halui_export_pin_IN_bit(&(halui_data->home_all), "halui2.home-all");
        if (retval < 0) return retval;
    }

    retval = halui_export_pin_IN_bit(&(halui_data->abort), "halui2.abort"); 
    if (retval < 0) return retval;

    for (joint=0; joint < num_axes ; joint++) {
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->joint_home[joint]), comp_id, "halui2.joint.%d.home", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->joint_unhome[joint]), comp_id, "halui2.joint.%d.unhome", joint);
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->joint_nr_select[joint]), comp_id, "halui2.joint.%d.select", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_plus[joint]), comp_id, "halui2.jog.%d.plus", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_minus[joint]), comp_id, "halui2.jog.%d.minus", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_float_newf(HAL_IN, &(halui_data->jog_analog[joint]), comp_id, "halui2.jog.%d.analog", joint); 
	if (retval < 0) return retval;
	retval =  hal_pin_float_newf(HAL_IN, &(halui_data->jog_increment[joint]), comp_id, "halui2.jog.%d.increment", joint);
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_increment_plus[joint]), comp_id, "halui2.jog.%d.increment-plus", joint);
	if (retval < 0) return retval;
	retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_increment_minus[joint]), comp_id, "halui2.jog.%d.increment-minus", joint);
	if (retval < 0) return retval;
    }

    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->joint_home[num_axes]), comp_id, "halui2.joint.selected.home"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->joint_unhome[num_axes]), comp_id, "halui2.joint.selected.unhome");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_plus[num_axes]), comp_id, "halui2.jog.selected.plus"); 
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_minus[num_axes]), comp_id, "halui2.jog.selected.minus"); 
    if (retval < 0) return retval;
    retval =  hal_pin_float_newf(HAL_IN, &(halui_data->jog_increment[num_axes]), comp_id, "halui2.jog.selected.increment");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_increment_plus[num_axes]), comp_id, "halui2.jog.selected.increment-plus");
    if (retval < 0) return retval;
    retval =  hal_pin_bit_newf(HAL_IN, &(halui_data->jog_increment_minus[num_axes]), comp_id, "halui2.jog.selected.increment-minus");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_float(&(halui_data->jog_speed), "halui2.jog-speed");
    if (retval < 0) return retval;
    retval = halui_export_pin_IN_float(&(halui_data->jog_deadband), "halui2.jog-deadband");
    if (retval < 0) return retval;


    for (int n=0; n<num_mdi_commands; n++) {
        retval = hal_pin_bit_newf(HAL_IN, &(halui_data->mdi_commands[n]), comp_id, "halui2.mdi-command-%02d", n);
        if (retval < 0) return retval;
    }
...
// Returns true if any of halui2.jog.N.plus, halui2.jog.N.minus, or
// halui2.jog.N.analog are true (where N is the passed-in joint number).
...
// Returns true if any of halui2.jog.selected.plus,
// halui2.jog.selected.minus, or halui2.jog.selected.analog are true.
// Otherwise, returns false.
