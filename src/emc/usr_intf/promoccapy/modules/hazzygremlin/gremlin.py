...
class StatCanon(glcanon.GLCanon, rs274.interpret.StatMixin):
    ...
    def change_tool(self, pocket):
        ...
        self.tool = rs274.interpret.StatMixin.get_tool(self,pocket)[0] #added for "run from line"

    #added for "run from line"
    run_from_line_parsed = False
    run_from_line_no = -1
    run_from_line_speed = -1
    run_from_line_mcodes = -1
    run_from_line_gcodes = -1
    tool = -1
    pos = -1
    ...
    def next_line(self, st):
        ...
        self.Gremlin_instance.fileloading(self.lineno)
        #added for "run from line"
        if self.run_from_line_no == self.lineno:
             self.run_from_line_parsed = True
             self.run_from_line_gcodes = self.state.gcodes
             self.run_from_line_mcodes = self.state.mcodes
             self.run_from_line_speed = self.state.speed
             self.run_from_line_feed = self.state.feed_rate
             self.run_from_line_tool = self.tool  
             self.pos=self.lo 
...
class Gremlin(gtk.gtkgl.widget.DrawingArea, glnav.GlNavBase,
    ...
    def get_modal(self, lineno, gcodes, linecount):
        ...
        canon.run_from_line_parsed = False
        canon.run_from_line_tool = -1
        ...
        startpos += "B%.4f " %(canon.pos[4]*inch) if "B" in self.get_geometry() else ""
        startpos += "C%.4f " %(canon.pos[5]*inch) if "C" in self.get_geometry() else ""
        startpos += "U%.4f " %(canon.pos[6]*inch) if "U" in self.get_geometry() else ""
        startpos += "V%.4f " %(canon.pos[7]*inch) if "V" in self.get_geometry() else ""
        startpos += "W%.4f " %(canon.pos[8]*inch) if "W" in self.get_geometry() else ""
        startpos += "\nG53 G0 Z[%.4f + %s]" %((canon.pos[2]*inch) if "Z" in self.get_geometry() else "", "#5403" if canon.run_from_line_gcodes[9]==430 else "0")
        lines=[]
        ...
        lines.append("G%s F%s" %(canon.run_from_line_gcodes[7]/10,
                                canon.run_from_line_feed))
        lines.append("G%s G%s G%s G%s G%s G%s G%s G%s G%s" 
                             %(canon.run_from_line_gcodes[3]/10,
                               canon.run_from_line_gcodes[4]/10,
                               canon.run_from_line_gcodes[5]/10,
                               canon.run_from_line_gcodes[6]/10,
                               canon.run_from_line_gcodes[8]/10,
                               canon.run_from_line_gcodes[9]/10,
                               canon.run_from_line_gcodes[10]/10,
                               canon.run_from_line_gcodes[14]/10.0,
                               canon.run_from_line_gcodes[15]/10),)
        lines.append(startpos.rstrip().lstrip())
        if canon.run_from_line_mcodes[4] > 0:
            lines.append("M%s" %canon.run_from_line_mcodes[4] )
        if canon.run_from_line_mcodes[5] > 0:
            lines.append("M%s" %canon.run_from_line_mcodes[5] )
        return lines


    def loading_finished(self, widget, result, seq):
        if self.getting_modal:
           return
        self.set_current_view()
        self.label.hide()
        try:
