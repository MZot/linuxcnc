...
class GlCanonDraw:
    ...
    def show_extents(self):
        ...
        if self.is_lathe():
            ...
            glPushMatrix()
            f = fmt % ((g.min_extents[x] - offset[x]) * dimscale * 2.0)
            f =  'f' + f
            glTranslatef(g.min_extents[x] - halfchar, y_pos, g.max_extents[z]+ pullback+ dashwidth)
            glRotatef(-90, 0, 0, 1)
            glRotatef(-90, 0, 1, 0)
            ...
            glTranslatef(0+offset[x]-charsize*1.2, y_pos, (g.max_extents[z] + g.min_extents[z])/2)
            glScalef(charsize, charsize, charsize)
            glRotatef(-90, 0, 0, 1)
            glRotatef(-90, 0, 1, 0)
            ...
            glTranslatef(0+offset[x]-dashwidth-charsize*1.2, y_pos, g.max_extents[z] - halfchar)
            glScalef(charsize, charsize, charsize)
            glRotatef(-90, 0, 0, 1)
            glRotatef(-90, 0, 1, 0)
            self.hershey.plot_string(f, 0, bbox)
            ...
            glTranslatef(0+offset[x]-dashwidth-charsize*1.2, y_pos, g.min_extents[z] + halfchar)
            glScalef(charsize, charsize, charsize)
            glRotatef(-90, 0, 0, 1)
            glRotatef(90, 0, 1, 0)
            glTranslatef(dashwidth*1.5, 0, 0)
    ...
    def dro_format(self,s,spd,dtg,limit,homed,positions,axisdtg,g5x_offset,g92_offset,tlo_offset):
            ...
            droposstrs = []
            used_letters = []
            for i in range(3):  #linuxcnc.MAX_AXIS):
                a = "XYZABCUVW"[i]
                if s.axis_mask & (1<<i):
