...
# Added keyboard support, used code from tooledit widget from Jim Craig
#
...
import sys, os, pango, linuxcnc, glib, uinput
...
class Offsetpage(gtk.VBox):
    ...
    def __init__(self, filename = None, *a, **kw):
        ...
        self.treeview1 = self.wTree.get_object("treeview2")
    ...
    def set_editing(self, widget):
        ...
        path, col = self.view2.get_cursor()
        columns = [c for c in self.view2.get_columns()]
        colnum = columns.index(col)
        if path == (0,):
          path=(3,)
        if colnum < 1:
          colnum = 10
        glib.timeout_add(50, self.select_cell,self.view2,path, columns[colnum])
    ...
    def col_editted(self, widget, filtered_path, new_text, col):
        ...
        try:
            self.store[row][col] = ("%10.03f " % eval(new_text)).lstrip() #locale.format("%10.4f", locale.atof(new_text))
        except:
        ...
        try:
            if not self.display_units_mm == self.program_units and not row == 2:
                if self.program_units == 1:
                    ...
                    convert = 1.0 / 25.4
                qualified = float(eval(new_text)) * convert
            else:
                qualified = float(eval(new_text))
        except:
            print 'error'
        ...
        try:
            ...
            if lncnc_running:
                ...
                if row == 1:
                    ...
                    if not pnum == None:
                        if self.axisletters[axisnum] == 'x' and 70 in self.status.gcodes[1:]: #G7 diameter mode
    ...
    def on_tree_navigate_key_press(self, treeview, event, filter):
        ...
        if keyname == 'Tab' or keyname == 'Right':
         if self.cell_edited == 0 and self.editing_mode == True: 
            cont = True
            cont2 = True
            i = 0
            while cont:
            ...
            if keyname == 'Right':
                ...
                if type(focuschild) is gtk.Entry:
                    self.col_editted(renderer[0], path, treeview.focus_child.props.text, colnum)
        ...
        elif keyname == 'Shift_R' or keyname == 'Shift_L': #select for editing
             if not self.cell_edited and self.editing_mode:
                glib.timeout_add(100, self.press_end)
                self.cell_edited = True
                glib.timeout_add(50, self.select_cell,treeview,path, columns[colnum])
