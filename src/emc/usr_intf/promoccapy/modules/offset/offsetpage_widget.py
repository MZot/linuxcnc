...
class Offsetpage(gtk.VBox):
    ...
    def col_editted(self, widget, filtered_path, new_text, col):
        ...
        try:
            self.store[row][col] = ("%10.03f " % self.s.eval(new_text)).lstrip() #locale.format("%10.4f", locale.atof(new_text))
        except:
        ...
        try:
            if not self.display_units_mm == self.program_units and not row == 2:
                if self.program_units == 1:
                    ...
                    convert = 1.0 / 25.4
                qualified = float(self.s.eval(new_text)) * convert
            else:
                qualified = float(self.s.eval(new_text))
        except:
            print 'error'
