            ...
            for s in tbl :
                ...
                if ";" in s:
                    ...
                    if tnumber != '0' :
                        ...
                        self.table.append([int(tnumber), tnumber, tdesc, torient, tdiam])
        ...
        if self.editdata_type in NUMBER_TYPES :
            ...
            try:
                ...
                if os.path.exists(self.param_value):
                    ...
                    filechooserdialog.set_current_folder(os.path.expanduser("~/LinuxCNC/nc_files"))      #MZ   (os.getcwd())
           ...
           print 'Features: Exception in do_start_editing()'
           print str(e)
...
class Feature(object):
    ...
    def include_gcode(self, srce) :  #MZ
        src = search_path(search_warning.dialog, srce, LIB_DIR)
        if src is not None:
            return io.open(src).read().replace('%','').replace('M2',';M2').replace('m2',';m2').replace('M30',';M30').replace('m30',';m30')
        return ''


    def set_tdiam(self, tn) :
        global TDIA
        try:
         for tool in TOOL_TABLE.table :
            if tool[1] == tn :
                TDIA = float (tool[4])
        except:
         TDIA = 0.0
...
#        self.cat_name = None
    ...
    def read(self, cat_name, read_all = True):
        ...
        if read_all :
            ...
            self.use_dual_views = read_boolean(config, 'layout', 'dual_view', False)
            self.side_by_side = read_boolean(config, 'layout', 'side_by_side', True)
            self.sub_hdrs_in_tv1 = read_boolean(config, 'layout', 'subheaders_in_master', True)
        ...
        self.use_pct = read_boolean(config, 'ngc', 'use_pct_signs', True)
        ...
        self.ngc_off_rot_coord_system = read_int(config, 'ngc', 'off_rot_coord_system', 7) #7 defaults to G59.3
    ...
    def create_defaults(self):
        ...
        self.default += _('(ncam.py -> self.defaults)\n\n')
        self.default += _('\n\n')
        ...
        self.default += ("/       (DEBUG, Turn on Skip Block [/])\n")
        self.default += ("/       M0\n")
        self.default += ("/       o<safety_9999> if [#<_task> ]\n")
        self.default += ("/         M30\n")
        self.default += ("/       o<safety_9999> endif\n")
...
class NCam(gtk.VBox):
    ...
    def on_label1_button_press_event(self, widget):
        print "click"
...
#        a.destroy
        ...
        self.add_catalog_items(self.menuAdd)
        self.menuAdd.append(gtk.SeparatorMenuItem())
        self.menuAdd.append(create_mi(self.actionLoadCfg))
        self.menuAdd.append(create_mi(self.actionImportXML))
...
#        webbrowser.open('https://www.youtube.com/channel/UCjOe4VxKL86HyVrshTmiUBQ')
                ...
                try:
                 if widget.row_expanded(path) :
                    ...
                    if depth > d_len :
                        ...
                        #widget.collapse_row(apath)  #ncam se zrusi ko gres levo v groupi, arrayu.
                        #print 'ok'
                except:
                    pass
    ...
    def save_file(self) :    #MZ Added save gcode and xml in single file
        xml = self.treestore_to_xml()
        f = open(self.file, 'w')
        f.write(self.to_gcode())
        f.write("\n")
        etree.ElementTree(xml).write(f, pretty_print = False)
        f.close()
        self.file_changed = False

    file=None
    def open_file(self, filename) :  #MZ read xml
        xmlstr = ''
        if int(os.path.getsize(filename)) == 0:  #prazen file
           self.new_from_template()
           self.clear_undo()
           self.file = filename
           fnstr=filename
           if len( fnstr ) > 30:
                fnstr = "..." + fnstr[len( fnstr ) - 29:len( fnstr )]
           self.builder.get_object("label1").set_label(fnstr)
           return
        with open(filename) as fp:
         for line in fp:
          if '<lcnc-ncam>' in line:
             xml = etree.ElementTree(etree.fromstring(line))
    ...
    def show_popup_add(self):
            self.menuAdd.show_all()
            self.menuAdd.popup(None, None, None,3,0)
            return
    ...
    def autorefresh_call(self, *arg) :
        return ############################# no autorefresh allowed MZ
        if not self.actionAutoRefresh.get_active() :
            return False
