...
class Filechooser(gobject.GObject):
    ...
    def on_vbox1_realize(self, widget):
        ...
        self.file_treeview.connect( "key_press_event", self.on_key_event, 1 )
        self.file_treeview.connect( "key_release_event", self.on_key_event, 0 )
    ...
    def on_key_event( self, widget, event, signal ):        
        ...
        #print("filechooser key = ",keyname)

        if keyname == "Right" and signal:
            self.file_treeview.row_activated(self.file_treeview.get_cursor()[0], self.file_treeview.get_cursor()[1] )
    ...
    def _fill_file_liststore(self, path=None):
        ...
        try:
         ...
         self._cur_dir='/home/linuxcnc/linuxcnc/nc_files'
         dirs = os.listdir(self._cur_dir)
        ...
        model.append([0, self.icons.get_for_directory('/'), "..", None, None])       #up one level MZ
    ...
    def on_filechooser_treeview_row_activated(self, widget, path, colobj):
        ...
        self.file_treeview.set_cursor(0,None)
    ...
    def copy_selected(self):
        files = [self.get_path_at_cursor()   ]  #self.get_selected()
        if files is None:
    ...
    def paste(self):
        ...
        if self._copy:
            for src in self._files:
                dst_name=self._copy_file(src, dst_dir)
        elif not self._copy:
            for src in self._files:
        ...
        for row in range(len(model)): #selektira novo datoteko
           if model[row][2] == dst_name:
             tree.set_cursor(row, None, False)
             tree.grab_focus()
             break
        return True
    ...
    def delete_at_cursor(self):   #MZ
       try:
        model = self.file_liststore
        tree = self.file_treeview
        cursor = tree.get_cursor()
        filename = self.get_path_at_cursor()

        for row in range(len(model)):
            if model[row][2] == self.file_liststore[self.file_treeview.get_cursor()[0]][2]:
                break
        focus_column = self.builder.get_object('col_file_name')
        model[row][0] = 1

        label = gtk.Label("Delete file: ")
        label.modify_font(pango.FontDescription("sans 20"))
        label2 = gtk.Label(self.file_liststore[self.file_treeview.get_cursor()[0]][2])
        label2.modify_font(pango.FontDescription("sans 20"))
        dialog = gtk.Dialog("Confirm delete",
                   None,
                   gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                   (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT,
                    gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        dialog.parse_geometry("400x200")
        ...
        if response == gtk.RESPONSE_ACCEPT:
            if os.path.isdir(filename):
               if os.listdir(filename) != []:
                           self.dialog = gtk.Dialog("Not empty dir",
                                       None,
                                      gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                                      (gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
                           self.dialog.parse_geometry("400x200")
                           self.dialog.vbox.pack_start(label)
                           label.set_text("Directory is not empty!")
                           label.show()
             ...
             move2trash(filename)
        self._fill_file_liststore()
        ln = len(model)-1 if cursor[0][0] == len(model) else cursor[0][0]
        tree.set_cursor(ln, None, True)
        tree.grab_focus()
       except:
        pass
    ...
    def rename_at_cursor(self):   #MZ
       try:
        model = self.file_liststore
        tree = self.file_treeview
        original_filename = self.get_path_at_cursor()
        label = gtk.Label("Enter new name: ")
        label.modify_font(pango.FontDescription("sans 20"))
        entry = gtk.Entry()
        entry.modify_font(pango.FontDescription("sans 20"))
        entry.set_text(os.path.basename( original_filename ))
        self.dialog = gtk.Dialog("Rename:",
                   None,
                   gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                   (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT,
                    gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        self.dialog.parse_geometry("400x200")
        self.dialog.vbox.pack_start(label)
        self.dialog.vbox.add(entry)
        entry.connect("activate", self.submit) #ko pritisnes "enter"
        ...
        if response == gtk.RESPONSE_ACCEPT:   
          if f != '':
            filename = os.path.join(self._cur_dir, f)
            if not os.path.exists(filename):
               os.rename(original_filename, filename)
               self._fill_file_liststore()
            else:
               self.dialog = gtk.Dialog("File exists",
                            None,
                            gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                            (gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
               self.dialog.parse_geometry("400x200")
               self.dialog.vbox.pack_start(label)
               label.set_text("File already exists!")
               entry.connect("activate", self.submit) #ko pritisnes "enter"
               label.show()
               response = self.dialog.run()
               self.dialog.destroy()
    ...
    def new_dir(self):
        label = gtk.Label("Enter new folder name: ")
        label.modify_font(pango.FontDescription("sans 20"))
        entry = gtk.Entry()
        entry.modify_font(pango.FontDescription("sans 20"))
        entry.set_text("new_folder")
        self.dialog = gtk.Dialog("New folder",
                   None,
                   gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                   (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT,
                    gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
          ...
          if f != '':
           try:
            ...
            if not os.path.isdir(filename):
               ...
               self._fill_file_liststore()
               self.set_cursor_at_path(filename)
            else:
               self.dialog = gtk.Dialog("Folder exists",
                            None,
                            gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                            (gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
               self.dialog.parse_geometry("400x200")
               self.dialog.vbox.pack_start(label)
               label.set_text("Folder already exists!")
               entry.connect("activate", self.submit) #ko pritisnes "enter"
               label.show()
               response = self.dialog.run()
               self.dialog.destroy()
    ...
    def new_file(self, type):
        label = gtk.Label("Enter new file name (*%s): " %type)
        label.modify_font(pango.FontDescription("sans 20"))
        entry = gtk.Entry()
        entry.modify_font(pango.FontDescription("sans 20"))
        entry.set_text("new_program")
        self.dialog = gtk.Dialog("New file",
                   None,
                   gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                   (gtk.STOCK_CANCEL, gtk.RESPONSE_REJECT,
                    gtk.STOCK_OK, gtk.RESPONSE_ACCEPT))
        ...
        if response == gtk.RESPONSE_ACCEPT:   
          if f != '':
           try:
            filename = os.path.join(self._cur_dir, f + type)
            if not os.path.exists(filename):
               os.mknod(filename)
               self._fill_file_liststore()
               self.set_cursor_at_path(filename)
